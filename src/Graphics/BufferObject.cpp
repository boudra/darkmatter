...
BufferObject::BufferObject(unsigned int size, unsigned int element_size, Type type)
    : m_id(0), m_capacity(size), m_element_size(element_size), m_type(buffer_type(type)), m_bound(false) {}
...
void BufferObject::release() { glBindBuffer(m_type, 0); m_bound = false; }
