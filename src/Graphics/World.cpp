...
namespace dm {
  ...
  bool find_common_side(const uint32_t a[3], const uint32_t b[3], uint32_t result[2][2])
  {
	...
	for(size_t j = i + 3; j < mesh.indices.size(); j += 3)
	  {
	    ...
	    if(find_common_side(&mesh.indices[i], &mesh.indices[j], common_side))
	      {

		const Vec3f pivot = (mesh.vertices[mesh.indices[i + common_side[0][0]]] + mesh.vertices[mesh.indices[i + common_side[1][0]]]) * 0.5f;

		const uint32_t outer_index_a = mesh.indices[i + 3 - (common_side[0][0] + common_side[1][0])];
		const uint32_t outer_index_b = mesh.indices[j + 3 - (common_side[0][1] + common_side[1][1])];
		...
		centroid.x += (((float)rand()/(float)RAND_MAX) - 0.5f) * 0.05f;
		centroid.y += (((float)rand()/(float)RAND_MAX) - 0.5f) * 0.05f;
  ...
  bool World::initialize() {
    ...
    size_t vbo_verts = terrain_.add_buffer<Vec3f>(BufferObject::Type::VBO, terrain_mesh_.vertices.size());
    size_t vbo_colors = terrain_.add_buffer<Vec4f>(BufferObject::Type::VBO, terrain_mesh_.colors.size());
    size_t vbo_normals = terrain_.add_buffer<Vec3f>(BufferObject::Type::VBO, terrain_mesh_.normals.size());
    size_t ibo = terrain_.add_buffer<unsigned int>(BufferObject::Type::IBO, terrain_mesh_.indices.size());

    terrain_.bind();

    terrain_.get_buffer(ibo).bind();
    terrain_.get_buffer(ibo).set_data<unsigned int>(&terrain_mesh_.indices[0], BufferObject::Usage::StaticDraw);

    terrain_.get_buffer(vbo_verts).bind();
    terrain_.add_attribute(vbo_verts, 0, 3, VertexObject::Primitive::Float, sizeof(Vec3f), 0);
    terrain_.get_buffer(vbo_verts).set_data<Vec3f>(&terrain_mesh_.vertices[0], BufferObject::Usage::StaticDraw);

    terrain_.get_buffer(vbo_colors).bind();
    terrain_.add_attribute(vbo_colors, 1, 4, VertexObject::Primitive::Float, sizeof(Vec4f), 0);
    terrain_.get_buffer(vbo_colors).set_data<Vec4f>(&terrain_mesh_.colors[0], BufferObject::Usage::StaticDraw);

    terrain_.get_buffer(vbo_normals).bind();
    terrain_.add_attribute(vbo_normals, 2, 3, VertexObject::Primitive::Float, sizeof(Vec3f), 0);
    terrain_.get_buffer(vbo_normals).set_data<Vec3f>(&terrain_mesh_.normals[0], BufferObject::Usage::StaticDraw);
  ...
  void World::component_added(const uint32_t component_type, Entity e) {
    //e.component<BodyComponent>().execute(Command::Type::GO_TO, Vec2i {20,20});
  }
  ...
  void World::update() {
    ...
    for (auto& body : bodies) {
      ...
      AnimationComponent* animation = e->has_component<AnimationComponent>() ?
	&e->component<AnimationComponent>() : nullptr;
	...
	case Command::Type::MOVE_UP: {
	  if(this->move(body, 0, 1)) {
	    if(animation) animation->animate("move_up", 1.0f, false, &unlock_body);
	    body.lock = true;
	  }
	  body.pop_command();
	} break;
	...
	case Command::Type::MOVE_DOWN: {
	  if(this->move(body, 0, -1)) {
	    if(animation) animation->animate("move_down", 1.0f, false, &unlock_body);
	    body.lock = true;
	  }
	  body.pop_command();
	} break;
	...
	case Command::Type::MOVE_RIGHT: {
	  if(this->move(body, 1, 0)) {
	    if(animation) animation->animate("move_right", 1.0f, false, &unlock_body);
	    body.lock = true;
	  }
	  body.pop_command();
	} break;
	...
	case Command::Type::MOVE_LEFT: {
	  if(this->move(body, -1, 0)) {
	    if(animation) animation->animate("move_left", 1.0f, false, &unlock_body);
	    body.lock = true;
	  }
	  body.pop_command();
	} break;
	...
	case Command::Type::GO_TO: {
	  ...
	  if((diff.x == 0 && diff.y == 0) || !m_map(destination).free)  {
	    body.pop_command();
	    break;
	  }
    ...
    for(auto& cursor : cursors) {
      ...
      Vec3f relative = selected_physics.size * 0.5f - cursor_physics.size * 0.5f; 
