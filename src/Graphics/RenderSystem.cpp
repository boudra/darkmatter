...
namespace dm {
  ...
  RenderSystem::RenderSystem()
    : SystemBase("Render", {SYSTEM(Camera), SYSTEM(AnimationSystem)}, {COMPONENT(Render2d)})  {}
  ...
  void RenderSystem::render(float interpolation) {
	...
	if(render.shadow && render.billboard && render.parent->has_component<PhysicsComponent>()) {
	  ...
	  if(!render.has_sprite("shadow")) {
	    render.add_sprite("shadow",
			      Vec3f{0.2f, 0.1f, 0.002f},
			      Vec2f{0.6f, 0.5f},
	  ...
	  // // m_shadow_sprite.scale = Vec3f{1.0f} + Vec3f{1.5f} * (physics.position.y * 9.5f);
	  // m_shadow_sprite.size = Vec2f{physics.size.x * 1.2f, physics.size.x * 0.5f};
    ...
    std::stringstream ss;
    ss << std::fixed << std::setprecision(2) << m_time << " ms" << " / " << 1000.0f / (float)m_time << " fps" << std::endl;
    ss << "Camera: " << m_camera->view() << std::endl;
    ss << "Cursor: " << m_cursor->component<BodyComponent>().position << std::endl;
    ss << "Sprites: " << MemoryPool<Sprite>::instance().size() << std::endl;
    ss << "Sprites dr: " << cnt << std::endl;

    std::string test = ss.str();

    // m_sprite_batch.draw(Vec2f(20.f, 679.f), "DroidSans 20", ss.str(), Color::Black);
    m_sprite_batch.draw(Vec2f(20.f, 680.f), "Monaco 20", ss.str(), Color::Yellow);

    m_sprite_batch.end();
  }

  Entity* RenderSystem::create_cursor(const std::string& texture, const Vec4i& color) {
  ...
  bool RenderSystem::initialize() {
    ...
    terrain_shader_.load("data/shaders/terrain_vertex.glsl", dm::Shader::Type::Vertex);
    terrain_shader_.load("data/shaders/terrain_frag.glsl", dm::Shader::Type::Fragment);
    ...
    m_projection.ortho({{-0.2f, -0.2f * 0.5625f}, Vec2f{0.2f, 0.2f * 0.5625f}}, {-1.0f, 100.0f});
    ...
    m_terrain.texture = GET_MANAGER(ResourceManager)->load_texture("data/wood.png");
    m_terrain.position = Vec3f(-160.0f * TILE_SIZE.x, -160.0f * TILE_SIZE.z, 0.0f);
    ...
    m_shadow_sprite.texture = GET_MANAGER(ResourceManager)->load_texture("data/shadow.png");
