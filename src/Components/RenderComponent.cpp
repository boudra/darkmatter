...
namespace dm {
   ...
   { 
      transform = matrix;
      for(auto& s : sprites)
	s.second->transform = matrix;
   }

  Sprite& Render2d::add_sprite(const std::string& name, const Vec3f& position, const Vec2f& size, const size_t& texture, const Vec4i& color)
   {
