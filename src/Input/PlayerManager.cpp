...
PlayerManager::PlayerManager() {
   //s_dispatcher->subscribe(this, &PlayerManager::button_pressed, EventType::KEY_PRESSED);
   s_dispatcher->subscribe(this, &PlayerManager::button_released, EventType::KEY_RELEASED);
   s_dispatcher->subscribe(this, &PlayerManager::mouse_motion, EventType::MOUSE_MOTION);
   s_dispatcher->subscribe(this, &PlayerManager::mouse_click, EventType::MOUSE_LPRESS);
}
...
void PlayerManager::mouse_click(const MouseEvent& e) {
   ...
   if(cursor.selected && map(m_hover).free)
   {
      ...
      if((m_hover - body.position).x < 0 && body.direction == Direction::RIGHT)
      {
         body_sprite.FlipHorizontal();
	 body.direction = Direction::LEFT;
      }
      else if((m_hover - body.position).x > 0 && body.direction == Direction::LEFT)
      {
         body_sprite.FlipHorizontal();
	...
	body.execute(Command::Type::GO_TO, m_hover, [animation_id, selected_entity]() {
            auto& animation = selected_entity->component<AnimationComponent>();
...
void PlayerManager::mouse_motion(const MouseEvent& e) {
   ...
   //const Vec3f weaponPosition = physics.GetPosition() + weapon.m_position + weapon.m_rotationOrigin;

   //Vec3f direction = mouseWorld - weaponPosition;
   //direction = direction.Normalize();

   //const float angle = std::atan2(direction.y, direction.x);
   const Vec3f playerCenter = physics.GetPosition() + physics.GetRotationOrigin();
   ...
   */
   ...
   Vec2i tile {int(std::floor(point.x / TILE_SIZE.x)), int(std::floor(point.y / TILE_SIZE.y))};
   ...
   physics.position = s_engine->system<World>()->real_position(m_hover, physics.size);
...
void PlayerManager::set_player(Entity* player) { 
   s_engine->system<RenderSystem>()->selected()->component<Cursor>().selected = player;
   m_player = player; 
}
