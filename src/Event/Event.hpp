...
struct MouseEvent : Event {
   MouseEvent(const Vec2f& position, const EventType type = EventType::MOUSE_MOTION) :
      Event(type), position(position) {}
   Vec2f position;
};

struct MouseWheelEvent : Event {
   MouseWheelEvent(const Vec2i& delta, const EventType type = EventType::MOUSE_WHEEL) :
