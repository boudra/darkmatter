...
ssize_t ResourceManager::load_texture(const std::string& filename, const std::string& name)
{
   const std::string& real_name = name.empty() ? filename.substr(filename.find_last_of("/") + 1) : name;
   ...
   Log::progress("debug", "Loading texture %s as %s", filename.c_str(), real_name.c_str());

   if(!texture.load(filename.c_str(), Texture::Filter::Nearest, Texture::Wrap::Clamp))
   {
...
ssize_t ResourceManager::load_font(const std::string& filename, size_t size)
{
   ...
   const std::string& real_name =
      filename.substr(name_pos + 1, filename.find_last_of(".") - 1 - name_pos) + " " + std::to_string(size);
...
Font* ResourceManager::get_font(const size_t& id)
{
   return &m_fonts[id];
}
