...
bool Texture::load(const char* filename, Filter filter_min,
                        Filter filter_mag, Wrap wrap_x, Wrap wrap_y) {
...
void Texture::set_data(unsigned char* data)
{
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, to_opengl_format(m_format), m_size.x, m_size.y, 0, format,
                GL_UNSIGNED_BYTE, data);
   
   if(m_format == Format::RGBA) {

      for (size_t i = bytes - 1; i < m_size.x * m_size.y * bytes; i += bytes)
      {
         if(data[i] < 255)  {
            m_transparent = true;
            break;      
