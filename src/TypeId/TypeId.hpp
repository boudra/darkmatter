...
#define dm_type_get(type)			\
  dm::global_type_info<dm_type_filter(type)>
...
namespace dm
{
  ...
  public:
    ...
    TypeInfo(const uint32_t id, const std::string name) : m_id(id), m_name(name)
    {
    }

    template <typename type>
    TypeInfo() : TypeInfo(global_type_info<type>::id, global_type_info<type>::name) {}
