...
namespace dm {
	...
	public:
		...
		Any()
			: m_info(dm_type_info(std::nullptr_t)), m_holder(nullptr)
		{
		}

		Any(const Any& other)
			: m_info(other.Type()), m_holder(other.m_holder ? other.m_holder->Clone() : nullptr)
		{
		}

		Any(Any&& other)
			: m_info(other.m_info), m_holder(other.m_holder)
		{
		...
		Any(const ValueType& data)
			: m_info(dm_type_info(ValueType)), m_holder(new Holder<typename std::decay<const ValueType>::type>(data))
		{
		}
		...
		Any(ValueType&& data,
			...
			m_holder(new Holder<typename std::decay<ValueType>::type>(std::forward<ValueType>(data)))
		{
		}
		...
		struct Holder : public HolderBase
		{
			...
			Holder(ValueType&& _data)
				: data(std::forward<ValueType>(_data))
			{
			}
