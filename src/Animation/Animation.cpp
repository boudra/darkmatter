...
ValueType interpolate(const AnimationFrameBase &current,
    ...
    const auto &currentFrame =
        static_cast<const AnimationFrame<ValueType> &>(current);
    const auto &nextFrame =
        static_cast<const AnimationFrame<ValueType> &>(next);
    return (nextFrame.value - currentFrame.value) * step;
}

void animate(AnimationComponent &c, const AnimationInstance &inst,
             const AnimationInfo &info, const AnimationFrameBase &current,
             const AnimationFrameBase &next, float step) {
    switch (info.type) {
        case AnimationType::PhysicsPosition: {
            auto &physics = c.parent->component<PhysicsComponent>();
            const Vec3f &new_pos =
                physics.position + interpolate<Vec3f>(current, next, (step));
        ...
        case AnimationType::PhysicsScale: {
            ...
            const Vec3f &new_scale =
                scale + interpolate<Vec3f>(current, next, (step));
        ...
        case AnimationType::SpritePosition: {
            ...
            const Vec3f &new_pos =
                sprite.position + interpolate<Vec3f>(current, next, (step));
        ...
        case AnimationType::SpriteColor: {
            auto &render = c.parent->component<Render2d>();
            auto &sprite = render.get_sprite(info.data.Cast<std::string>());
            const Vec4i &new_color =
                sprite.color + interpolate<Vec4i>(current, next, (step));
        ...
        case AnimationType::SpriteTexture: {
            ...
            const auto &current_frame =
                static_cast<const AnimationFrame<Vec2i> &>(current);
            sprite.crop.position =
                sprite.crop.size * Vec2f{current_frame.value};
        } break;
        ...
        case AnimationType::SpriteScale: {
            ...
            auto &sprite = render.get_sprite(info.data.Cast<std::string>());
            const Vec3f &new_scale =
                sprite.scale + interpolate<Vec3f>(current, next, (step));
...
size_t AnimationSystem::start_animation(
    const std::string &name, float speed, bool loop,
