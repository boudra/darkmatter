...
namespace dm
{
  ...
  public:
    ...
    {
      object_type* new_block =
	reinterpret_cast<object_type*>(operator new(sizeof(object_type) * elements,
						    std::nothrow));
                                          ...
                                          const size_t new_size)
    {
      ...
      if(ptr) {
	std::memcpy(new_block, ptr, sizeof(object_type) * std::min(size, new_size));
	deallocate(ptr);
      }
  ...
  public:
    ...
    void sort() {
      std::sort(data_, data_ + top_, std::greater<object_type>());
    }
  ...
  public:
    ...
    object_type& push(arg_types&&... args)
    {
      ...
      if(new_object) new (new_object) object_type(std::forward<arg_types>(args)...);
      return *new_object;
    }
	...
	last_block_ = block;
      ...
      for (object_type* i = block->data_; i < block->data_ + block_size;
	   i++)
	free_.push(i);
    ...
    {
      return std::binary_search(free_.begin(), free_.end(), object, std::greater<object_type*>());
    }
    ...
    object_type* end_ptr() {
      return &last_block_->data_[block_size];
    }
